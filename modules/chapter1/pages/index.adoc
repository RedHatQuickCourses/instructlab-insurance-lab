...
. Monitor the GPU:
* Open first terminal and execute the nvtop (NVIDIA TOP) command to immerse yourself in real-time GPU insights. This will allow you to keep a close watch on GPU utilization throughout the lab, ensuring you have a comprehensive understanding of your GPU's performance.
+
[subs="+quotes,+macros"]
----
...
. Set up your lab environment for Parasol Insurance application and InstructLab:
* Open second terminal and begin by creating the `~/install.sh` script using `vi` or any other editor of your choice to set up your lab environment.
+
[subs="+quotes,+macros"]
----
...
. Initialse and set up the InstructLab:
* Open third terminal and navigate to the InstructLab directory:
+
[subs="+quotes,+macros"]
----
...
* Switch back to second terminal and edit the `~/.bashrc` file to add the following line.
+
[subs="+quotes,+macros"]
----
...
* Change directory to `~/parasol-insurance/app`.
+
[subs="+quotes,+macros"]
----
